...
export function Products(){
    ...
    return(
        <Container>
            ...
            <div style={{marginTop:'4vw',}}>{typeof card !== "undefined" && card.map((value)=>{return <Card><input type="checkbox" /><a>{value.cod}</a><a>{value.nome}</a><a>{value.fornecedor}</a><a>{value.qtd}</a><a>R$ {value.preco}</a></Card>})}</div>
